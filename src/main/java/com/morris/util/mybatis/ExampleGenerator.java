...
public class ExampleGenerator extends AbstractJavaGenerator {
    ...
    private InnerClass getGeneratedCriteriaInnerClass(
        ...
        if (introspectedTable.hasJDBCDateColumns()) {
            ...
            // 引入 import java.text.SimpleDateFormat;
            topLevelClass.addImportedType("java.text.SimpleDateFormat");
    ...
    private Method getSingleValueMethod(IntrospectedColumn introspectedColumn,
        ...
        if (introspectedColumn.isJDBCDateColumn() || "TIMESTAMP".equalsIgnoreCase(introspectedColumn.getJdbcTypeName())) {
        	method.addBodyLine("SimpleDateFormat sdf = new SimpleDateFormat(\"yyyy-MM-dd\");");
        ...
        if (introspectedColumn.isJDBCDateColumn() || "TIMESTAMP".equalsIgnoreCase(introspectedColumn.getJdbcTypeName())) {
            sb.append("to_char(" + MyBatis3FormattingUtilities
                    .getAliasedActualColumnName(introspectedColumn) + ",'yyyy-mm-dd')");
            sb.append(' ');
            sb.append(operator);
            sb.append("\", "); //$NON-NLS-1$
            sb.append("sdf.format(value)"); //$NON-NLS-1$
        }else {
    ...
    private Method getSetBetweenOrNotBetweenMethod(
        ...
        if (introspectedColumn.isJDBCDateColumn() || "TIMESTAMP".equalsIgnoreCase(introspectedColumn.getJdbcTypeName())) {
            ...
            sb.append("\", "); //$NON-NLS-1$
            sb.append("sdf.format(value1), sdf.format(value2)"); 
        }else {
        	sb.append(MyBatis3FormattingUtilities
                    .getAliasedActualColumnName(introspectedColumn));
            if (betweenMethod) {
